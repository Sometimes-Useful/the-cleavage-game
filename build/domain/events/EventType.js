"use strict";
/* eslint-disable no-unused-vars */
exports.__esModule = true;
exports.EventType = void 0;
var EventType;
(function (EventType) {
    EventType["CONNECT_CHAT"] = "CONNECT_CHAT";
    EventType["DISCONNECT_CHAT"] = "DISCONNECT_CHAT";
    EventType["NEW_CLEAVAGE"] = "NEW_CLEAVAGE";
    EventType["LAUNCH_CLEAVAGE"] = "LAUNCH_CLEAVAGE";
    EventType["PLAYER_CLEAVE"] = "PLAYER_CLEAVE";
    EventType["PLAYER_MESSAGE"] = "PLAYER_MESSAGE";
    EventType["DRAW_CLEAVAGE"] = "DRAW_CLEAVAGE";
    EventType["PLAYER_SUGGEST_CLEAVAGE"] = "SUGGEST_CLEAVAGE";
    EventType["PLAYER_ASK_FOR_HELP"] = "PLAYER_ASK_FOR_HELP";
    EventType["CANCEL_CLEAVAGE"] = "CANCEL_CLEAVAGE";
    EventType["PLAYER_HYPERLIKE"] = "PLAYER_HYPERLIKE";
    EventType["PLAYER_APPLAUSE"] = "PLAYER_APPLAUSE";
    EventType["PLAYER_SHOOT"] = "PLAYER_SHOOT";
    EventType["PLAYER_WHISTLE"] = "PLAYER_WHISTLE";
    EventType["APPLICATION_START"] = "APPLICATION_START";
    EventType["NAVIGATE"] = "NAVIGATE";
    EventType["CHANGE_MUSIC_VOLUME"] = "CHANGE_MUSIC_VOLUME";
    EventType["CHANGE_SOUND_VOLUME"] = "CHANGE_SOUND_VOLUME";
    EventType["PLAYER_QUIT"] = "PLAYER_QUIT";
    EventType["START_AUTOPLAY"] = "START_AUTOPLAY";
    EventType["CHECK_AUTOPLAY"] = "CHECK_AUTOPLAY";
    EventType["STOP_AUTOPLAY"] = "STOP_AUTOPLAY";
    EventType["DRAW_GLOBAL_CLEAVAGE_DRAWPILE"] = "DRAW_GLOBAL_CLEAVAGE_DRAWPILE";
    EventType["SAVE_CLEAVAGE_ON_GLOBAL_CLEAVAGE_DRAWPILE"] = "SAVE_CLEAVAGE_ON_GLOBAL_CLEAVAGE_DRAWPILE";
    EventType["PLAYER_JOIN_BAR"] = "PLAYER_JOIN_BAR";
    EventType["PLAYER_MOVE"] = "PLAYER_MOVE";
    EventType["INSTALL_NEW_TABLE"] = "INSTALL_NEW_TABLE";
    EventType["INSTALL_NEW_STOOLS_ON_TABLE"] = "INSTALL_NEW_STOOLS_ON_TABLE";
    EventType["TABLE_STOOL_AVAILABLE"] = "TABLE_STOOL_AVAILABLE";
    EventType["CREATE_BAR"] = "CREATE_BAR";
    EventType["DRAW"] = "DRAW";
    EventType["INSTALL_NEW_STOOLS_ON_BAR"] = "INSTALL_NEW_STOOLS_ON_BAR";
    EventType["CHANGE_GAME_PHASE"] = "CHANGE_GAME_PHASE";
    EventType["VIDEO_EXTRACT_START"] = "VIDEO_EXTRACT_START";
    EventType["VIDEO_EXTRACT_STOP"] = "VIDEO_EXTRACT_STOP";
    EventType["CHANGE_VIDEO_EXTRACT_VOLUME"] = "CHANGE_VIDEO_EXTRACT_VOLUME";
    EventType["ERASE"] = "ERASE";
    EventType["REFRESH_CLEAVAGE_DRAWPILE_QUANTITY"] = "REFRESH_CLEAVAGE_DRAWPILE_QUANTITY";
    EventType["RETRIEVE_CLEAVAGE_DRAWPILE_QUANTITY"] = "RETRIEVE_CLEAVAGE_DRAWPILE_QUANTITY";
    EventType["REGISTER_STREAMER"] = "REGISTER_STREAMER";
    EventType["REGISTER_STREAMER_BACKEND"] = "REGISTER_STREAMER_BACKEND";
    EventType["RETRIEVE_STREAMER"] = "RETRIEVE_STREAMER";
    EventType["REFRESH_REGISTERED_STREAMERS"] = "REFRESH_REGISTERED_STREAMERS";
    EventType["RETRIEVE_STREAMERS"] = "RETRIEVE_STREAMERS";
    EventType["CHECK_REGISTERED_STREAMER"] = "CHECK_REGISTERED_STREAMER";
})(EventType = exports.EventType || (exports.EventType = {}));
