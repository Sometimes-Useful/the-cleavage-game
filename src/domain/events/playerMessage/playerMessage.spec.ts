...
feature(EventType.PLAYER_MESSAGE, [
    clientScenario(`Scenario 1 : ${applicationMessagePrefix + AuthorizedMessage.LEFT}`, [
        app => whenEventOccurs(app, new PlayerMessageEvent(player1().username, applicationMessagePrefix + AuthorizedMessage.LEFT)),
        app => theEventIsSent(Gherkin.THEN, app, new PlayerCleaveEvent(player1().username, PlayerCleave.LEFT))
    ]),
    clientScenario(`Scenario 2 : ${applicationMessagePrefix + AuthorizedMessage.RIGHT}`, [
        app => whenEventOccurs(app, new PlayerMessageEvent(player1().username, applicationMessagePrefix + AuthorizedMessage.RIGHT)),
        app => theEventIsSent(Gherkin.THEN, app, new PlayerCleaveEvent(player1().username, PlayerCleave.RIGHT))
    ]),
    clientScenario(`Scenario 3 : ${applicationMessagePrefix + AuthorizedMessage.UNSUPPORTED}`, [
        app => whenEventOccurs(app, new PlayerMessageEvent(player1().username, applicationMessagePrefix + AuthorizedMessage.UNSUPPORTED)),
        app => theChatGatewaySendMessageToPlayer(Gherkin.THEN, app, new MessageForPlayer(player1().username, dontKnowWhatToDoWithThatMessage(player1().username)))
    ]),
    clientScenario(`Scenario 4 : ${AuthorizedMessage.UNSUPPORTED}`, [
        app => whenEventOccurs(app, new PlayerMessageEvent(player1().username, AuthorizedMessage.UNSUPPORTED)),
    ...
    clientScenario(`Scenario 5 : ${applicationMessagePrefix + AuthorizedMessage.RIGHT} with another player ${JSON.stringify(player2)}`, [
        app => whenEventOccurs(app, new PlayerMessageEvent(player2().username, applicationMessagePrefix + AuthorizedMessage.RIGHT)),
        app => theEventIsSent(Gherkin.THEN, app, new PlayerCleaveEvent(player2().username, PlayerCleave.RIGHT))
    ]),
    clientScenario(`Scenario 6 : ${`${applicationMessagePrefix}${AuthorizedMessage.SUGGEST_CLEAVAGE} ${cleavageTitle1}`}`, [
        app => whenEventOccurs(app, new PlayerMessageEvent(player1().username, `${applicationMessagePrefix}${AuthorizedMessage.SUGGEST_CLEAVAGE} ${cleavageTitle1}`)),
        app => theEventIsSent(Gherkin.THEN, app, new PlayerSuggestCleavageEvent(player1().username, new Cleavage({ title: cleavageTitle1, leftChoice: { name: 'Gôche', players: [] }, rightChoice: { name: 'Drouate', players: [] }, players: [] })))
    ]),
    clientScenario(`Scenario 7 : ${`${applicationMessagePrefix}${AuthorizedMessage.HELP}`}`, [
        app => whenEventOccurs(app, new PlayerMessageEvent(player1().username, `${applicationMessagePrefix}${AuthorizedMessage.HELP}`)),
        app => theEventIsSent(Gherkin.THEN, app, new PlayerAskForHelpEvent(player1().username))
    ]),
    clientScenario(`Scenario 8 : ${`${applicationMessagePrefix}${AuthorizedMessage.SHORT_HELP}`}`, [
        app => whenEventOccurs(app, new PlayerMessageEvent(player1().username, `${applicationMessagePrefix}${AuthorizedMessage.SHORT_HELP}`)),
        app => theEventIsSent(Gherkin.THEN, app, new PlayerAskForHelpEvent(player1().username))
    ]),
    clientScenario(`Scenario 9 : ${`${applicationMessagePrefix}${AuthorizedMessage.SHORT_APPLAUSE}`}`, [
        app => whenEventOccurs(app, new PlayerMessageEvent(player1().username, `${applicationMessagePrefix}${AuthorizedMessage.SHORT_APPLAUSE}`)),
    ...
    clientScenario(`Scenario 10 : ${`${applicationMessagePrefix}${AuthorizedMessage.SHORT_SHOOT}`}`, [
        app => whenEventOccurs(app, new PlayerMessageEvent(player1().username, `${applicationMessagePrefix}${AuthorizedMessage.SHORT_SHOOT}`)),
    ...
    clientScenario(`Scenario 11 : ${`${applicationMessagePrefix}${AuthorizedMessage.SHORT_HYPERLIKE}`}`, [
        app => whenEventOccurs(app, new PlayerMessageEvent(player1().username, `${applicationMessagePrefix}${AuthorizedMessage.SHORT_HYPERLIKE}`)),
    ...
    clientScenario(`Scenario 12 : ${`${applicationMessagePrefix}${AuthorizedMessage.SHORT_WHISTLE}`}`, [
        app => whenEventOccurs(app, new PlayerMessageEvent(player1().username, `${applicationMessagePrefix}${AuthorizedMessage.SHORT_WHISTLE}`)),
